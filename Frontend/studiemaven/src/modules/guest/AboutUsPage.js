...
const icons = [
  { label: "Award 1", icon: certificateOne },
  { label: "Award 2", icon: certificateTwo },
  { label: "Award 3", icon: certificateThree },
  { label: "Award 4", icon: certificateFour }
];
...
const AwardsPage = () => {
  ...
  // For scroll-based animation update (if using Locomotive or similar)
            ...
            {icons.map((icon, idx) => (
              <Col xs={6} md={3} key={icon.label}>
                <motion.div
                  ...
                  onViewportEnter={triggerScrollUpdate}
                >
                  <img src={icon.icon} alt={icon.label} style={{ width: "100%", borderRadius: "10px" }} />
                  <div style={{ textAlign: "center", marginTop: 10 }}>{icon.label}</div>
                </motion.div>
              </Col>
            ))}
          </Row>
